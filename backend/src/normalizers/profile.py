...
def apply_profile(raw: dict, profile: SourceProfile) -> NormalizeResult:
    ...
    for i, rule in enumerate(rules):
        ...
        if fm.service:
            proto, port, label = parse_service(service=rule.get(fm.service))
        else:
            proto, port, label = parse_service(port=rule.get(fm.port) if fm.port else None,
                                               protocol=rule.get(fm.protocol) if fm.protocol else None)
