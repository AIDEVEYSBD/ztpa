...
import { ToolBadge, Tag, Skeleton, SkeletonText, Spinner } from "./ui";
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
      <div className="panel p-4">
        <div className="mb-3 flex flex-wrap items-center gap-2 text-sm font-semibold">
          <Layers size={16} className="text-text2" /> Unified assets ({t.rows.length}{t.rows.length !== assets.length ? ` of ${assets.length}` : ""})
          <SearchBox value={t.q} onChange={t.setQ} placeholder="Filter assets, tags, IPs…" className="sm:ml-auto" />
        </div>
        <p className="mb-3 text-xs text-muted">One identity per asset. IP is an attribute, not the key. Multi-tool assets were correlated deterministically (exact name or shared IP).</p>
                ...
                <tr key={a.asset_id} className="border-b last:border-0">
                  <td className="py-1.5 pr-3 font-medium">
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      {merged.has(a.asset_id) && <Link2 size={12} className="shrink-0 text-accent" aria-label="cross-tool correlated" />}
                    </span>
                  </td>
                  <td className="pr-3 text-xs text-muted">{a.kind}</td>
                  <td className="space-x-1 pr-3">{a.tags.map((t) => <Tag key={t}>{t}</Tag>)}</td>
                  <td className="pr-3 font-mono text-[11px] text-muted">{a.ip_set.join(", ") || "-"}</td>
                  <td className="space-x-1">{a.source_tools.map((t) => <ToolBadge key={t} tool={t} />)}</td>
                </tr>
              ))}
              {!loading && t.rows.length === 0 && (
                <tr><td colSpan={5} className="py-6 text-center text-xs text-muted">No assets match this filter.</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2 text-sm font-semibold"><GitMerge size={16} className="text-text2" /> Possible duplicates (review)</div>
        <p className="text-xs text-muted">Embedding-based suggestions for a human to confirm. The engine never auto-merges (a wrong merge corrupts a fact).</p>
        {sugg === undefined ? (
          <div className="space-y-3">
        ...
        ) : sugg.length === 0 ? <div className="panel p-3 text-xs text-muted">No candidates. All identities resolved deterministically.</div> :
            sugg.map((s, i) => {
              const busy = merging === `${s.a}~${s.b}`;
              return (
                <div key={i} className="panel p-3">
                  <div className="flex flex-wrap items-center gap-2 text-sm font-medium">
                    <span className="mono break-all">{s.a}</span><span className="text-muted">~</span><span className="mono break-all">{s.b}</span>
                    <span className="ml-auto chip border-accent bg-accent-soft text-text2">{(s.confidence * 100).toFixed(0)}%</span>
                  </div>
                  <div className="mt-1 flex flex-wrap gap-1">{s.shared_sensitive_tags.map((t) => <Tag key={t}>{t}</Tag>)}</div>
                  <div className="mt-2 text-xs text-muted"><Prose className="!text-xs">{s.reason}</Prose></div>
